...
import { X, Mail, Lock, User, Eye, EyeOff, ArrowRight, Sparkles, Loader2, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function AuthModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async e => {
    ...
    if (tab === 'signup') {
      ...
      setLoading(true);
      try {
        await login({ email: signinEmail.trim().toLowerCase(), password: signinPass });
        setSuccess(true);
        setTimeout(onClose, 800);
      } catch (err) {
          ...
          >
            ...
            <AnimatePresence>
              ...
              {success && (
                ...
                >
                  <CheckCircle2 size={14} /> Success! Redirecting…
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: '10px', cursor: (loading || googleLoading) ? 'not-allowed' : 'pointer',
                fontFamily: 'var(--font-sans)',
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginTop: '4px' }}>
              ...
              >
                {loading ? <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> : (tab === 'signup' ? 'Create Account' : 'Sign In')}
